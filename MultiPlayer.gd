...
#multiplayer varables
slave var slave_position = Vector3()
slave var slave_movement = Movement
onready var cam = $Boddy/Head/Camera
...
func Priamry_fire():
	...
	Primary_countdown += 1
	if is_network_master() and Input.is_action_pressed("Primary") and Primary_countdown >= Gear.Primary_rof:
		Primary_countdown = 0
		rpc('shoot')
...
#multiplayer function for sending game data
